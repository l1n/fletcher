...
class CommandHandler:
    ...
    async def command_handler(self, message):
        ...
        if user == client.user:
            logger.info(f"{message.id} #{message.guild.name if message.guild else message.channel.recipient.name}:{message.channel.name} <{user.name}:{user.id}> [{sent_com_score}] {message.system_content}",
                    extra={
