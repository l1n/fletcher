...
async def restorerole_function(member, client, config):
    try:
        ...
        logging.info(f'RPR: Restoring roles {",".join([str(role) for role in roles])} for {member.id} in {member.guild.id}')
        await member.edit(nick=name, roles=roles, reason='Restoring Previous Roles')
    except Exception as e:
        if cur is not None:
            conn.rollback()
        exc_type, exc_obj, exc_tb = exc_info()
        logging.error(f'RPR[{exc_tb.tb_lineno}]: {type(e).__name__} {e}')
...
async def saverole_function(member, client, config):
    try:
        ...
        if len(member.roles):
            ...
            logging.info(f'SRF: Storing roles {",".join([str(role) for role in roles])} for {member.id} in {member.guild.id}')
            cur = conn.cursor()
        ...
        if cur is not None:
            conn.rollback()
        exc_type, exc_obj, exc_tb = exc_info()
        logging.error(f'SRF[{exc_tb.tb_lineno}]: {type(e).__name__} {e}')
...
async def lockout_function(member, client, config):
    try:
        if type(member) is discord.Member:
            for category, channels in member.guild.by_category():
                if category is not None:
                    logging.debug("LOF: "+str(member)+" from category "+str(category)+" in "+str(member.guild))
                    ...
                    for channel in channels:
                        logging.debug("LOF: "+str(member)+" from non-category channel "+str(channel)+" in "+str(member.guild))
        ...
        exc_type, exc_obj, exc_tb = exc_info()
        logging.error(f'LOF[{exc_tb.tb_lineno}]: {type(e).__name__} {e}')
...
async def randomize_role_function(member, client, config):
    try:
        if type(member) is discord.Member:
            ...
            logging.info("RRF: adding role "+str(role)+" to "+str(member))
            await member.add_roles(role, reason='Per Randomize Role Guild setting (see randomize_role_list)', atomic=False)
    except Exception as e:
        exc_type, exc_obj, exc_tb = exc_info()
        logging.error(f'RRF[{exc_tb.tb_lineno}]: {type(e).__name__} {e}')

async def printhello_reload_function(guild, client, config):
    logging.debug("PHRF: Hello to guild "+guild.name+" at "+str(datetime.utcnow())+"!")

async def chanban_join_function(member, client, config):
    logging.info('CBJF: '+str(member)+' '+str(config['chanban_younger_than'])+' '+str(member.guild.get_channel(int(config['chanban_channel'])).name))
...
async def chanban_reload_function(guild, client, config):
    try:
        ...
        logging.debug('CBRF: '+str(config['chanban_younger_than'])+' '+channel.name)
        now = datetime.utcnow()
        ...
        for member in younglings:
            if member not in member_overrides:
                logging.info('CBRF: Banning '+str(member))
        ...
        for member in member_overrides:
            ...
            if now - member.joined_at > age_of_consent and now - member.joined_at < age_of_consent_expiry and permissions.read_messages == False and permissions.send_messages == False and permissions.embed_links == False:
                logging.info('CBRF: Unbanning '+str(member))
                await channel.set_permissions(member, overwrite=None)
    except Exception as e:
        exc_type, exc_obj, exc_tb = exc_info()
        logging.error(f'CBRF[{exc_tb.tb_lineno}]: {type(e).__name__} {e}')
        ...
        if not allowed:
            ...
            if config.get('regex-kill') == "On":
                ...
                try:
                    ...
                    logging.warning("MRF: Forbidden to delete message in "+str(message.channel))
                    pass
        return allowed
    except Exception as e:
        exc_type, exc_obj, exc_tb = exc_info()
        logging.error(f'MRF[{exc_tb.tb_lineno}]: {type(e).__name__} {e}')
