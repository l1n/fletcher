...
async def saverole_function(member, client, config):
    try:
        ...
        if len(member.roles):
            ...
            logger.info(f'SRF: Storing roles {",".join([str(role) for role in roles])} for {member.id} in {member.guild.id}')
            cur = conn.cursor()
        ...
        if type(member) is discord.Member:
            for category, channels in member.guild.by_category():
                if category is not None:
                    logger.debug("LOF: "+str(member)+" from category "+str(category)+" in "+str(member.guild))
                    ...
                    for channel in channels:
                        logger.debug("LOF: "+str(member)+" from non-category channel "+str(channel)+" in "+str(member.guild))
...
async def printhello_reload_function(guild, client, config):
    logger.debug("PHRF: Hello to guild "+guild.name+" at "+str(datetime.utcnow())+"!")

async def chanban_join_function(member, client, config):
    logger.info('CBJF: '+str(member)+' '+str(config['chanban_younger_than'])+' '+str(member.guild.get_channel(int(config['chanban_channel'])).name))
...
async def chanban_reload_function(guild, client, config):
    try:
        ...
        logger.debug('CBRF: '+str(config['chanban_younger_than'])+' '+channel.name)
        now = datetime.utcnow()
        ...
        if not allowed:
            ...
            if config.get('regex-kill') == "On":
                ...
                try:
                    ...
                    logger.warning("MRF: Forbidden to delete message in "+str(message.channel))
                    pass
        return allowed
    except Exception as e:
        exc_type, exc_obj, exc_tb = exc_info()
