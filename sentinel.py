    ...
    try:
        ...
        if nameAutoGenerated:
            return await message.channel.send(
                "Banner created! `!pledge {}` to commit to this pledge.".format(
                    str(bannerId)
                )
            )
        else:
...
async def pledge_function(message, client, args):
    try:
        ...
        if cur.fetchone()[0]:
            conn.commit()
            return await message.channel.send(
                "You already committed to this banner. `!defect {}` to defect.".format(
                    " ".join(args)
                )
            )
        else:
    ...
    try:
        ...
        if cur.fetchone()[0]:
            ...
            if bannerInfo[0] >= bannerInfo[1]:  # Triggered banner, can't go back now
                conn.commit()
                return await message.channel.send(
                    "Critical mass reached for banner {}! You can't defect anymore.".format(
                        bannerInfo[2]
                    )
                )
            else:
