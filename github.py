...
async def github_search_function(message, client, args):
    try:
        if "Guild "+str(message.guild.id) in config:
            ...
            raise Exception("No guild-specific configuration for source code management on guild "+str(message.guild))
        ...
        async with aiohttp.ClientSession(headers={'Authorization': 'token '+config['github']['personalAccessToken']}) as session:
            ...
            async with session.get(url, params={'q': " ".join(args)+" repo:"+scoped_config['github-repo']}) as response:
                ...
                if response_body['total_count'] == 0:
                    ...
                    for issue in response_body['items']:
                        await message.channel.send(embed=issue_to_embed(issue, issue['user']['login']))
                else:
