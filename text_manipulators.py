...
async def rot13_function(message, client, args):
    try:
        if len(args) == 2 and type(args[1]) is discord.User:
            return await args[1].send("Spoiler from conversation in #{} ({}) <https://discordapp.com/channels/{}/{}/{}>:\n{}".format(message.channel.name, message.channel.guild.name, message.channel.guild.id, message.channel.id, message.id, codecs.encode(message.content, 'rot_13')))
...
async def memfrob_function(message, client, args):
    try:
        if len(args) == 2 and type(args[1]) is discord.User:
            return await args[1].send("Spoiler from conversation in #{} ({}) <https://discordapp.com/channels/{}/{}/{}>:\n{}".format(message.channel.name, message.channel.guild.name, message.channel.guild.id, message.channel.id, message.id, memfrob(message.content.split(': ', 1)[1]).replace("\n"," ")))
        else:
